    ...
    try {
      ...
      if (success) {
        ...
        setTimeout(() => {
          ...
          if (username === 'admin') {
            navigate('/hotel');
          } else if (username.startsWith('servant') || username === 'waiter') {
            navigate('/wk/take-order');
          } else if (username.startsWith('kitchen')) {
            navigate('/wk/kitchen');
          } else if (username.startsWith('cashier')) {
            navigate('/wk/billing');
          }
        }, 1500);
      } else {
      ...
      setIsLoading(false);
      showMessage('Authentication failed. Please check your credentials and try again.', 'error');
      setShake(true);
      setTimeout(() => setShake(false), 300);
    }
  };
